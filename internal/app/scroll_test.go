...
func TestAPaneLabelSlidesToo(t *testing.T) {
	// Sliding says how a name too wide for its row is fitted, and a pane's
	// label is fitted by the same decorator a title is.
	cfg := paneConfig()
	cfg.Scroll = true
	...
	h := startConfigured(t, herdrtest.New(
		...
		[]herdr.PaneInfo{
			...
			{PaneID: "wE:p2", TabID: "wE:t1", CWD: herdrtest.Dir("work", wide)},
		},
	), cfg)
	...
	assertSlid(t, labelsOf(h, "wE:p2"), polls, resolver.DefaultScrollStep)
}
