...
because fitting is its job too. It fits a pane's label as well, and no position
leads one: the key in front of a title switches to a tab, and there is none that
switches into a split.

## Fitting: cut or slide

A name wider than `MaxLength` is cut by default — the tail goes, and no
...
with a `Fit` (`internal/resolver/fitted.go`), a function from an id, a name
and a width to what the row shows, and truncation is the one it uses when
given none. `Slide` (`internal/resolver/slide.go`) is a pure function of the
name, the width, a tick and the step; the tick per tab and per pane lives in
the poll loop, which is what carries state between polls (see [the poll
loop](poll-loop.md)). Sliding renames every poll it moves, which is the rate
the loop already bounds renames to. A poll that only moved the window logs a
slide at DEBUG rather than a rename, so the log stays about the names that
changed.
