...
Every setting Auto Title has is one of twelve `HERDR_AUTO_TITLE_*` variables,
...
builds the resolver chain — `MAX_LENGTH`, `BRANCH_MAX` and `POSITION` are baked
into `resolver.Default`, `SCROLL` and `SCROLL_STEP` into the fit it is handed —
so rereading the file mid-run would apply some settings and quietly ignore
others.
